...
static void dxx_tracker_reqgames( Json::Value &data )
{
	...
	for( unsigned i = 0 ; i < data.size() ; i++ )
	{
		...
		// This ntstring shit is fucking awful
		...
		// Fill it in
		if( udp_dns_filladdr( addr, sHost.c_str(), iPort ) < 0 )
			continue;
...
#define DXX_SECRET_LEVEL_FORMAT_ARGUMENT
	 ...
	 * change ngp file.
	 */
	if( GameArg.MplTrackerHost.empty() )
		Netgame.Tracker = 0;
#endif
