...
struct dxx_curl_handle
{
	dxx_curl_handle() : iPos( 0 ), cb( NULL ), sPost( "" ) { }
...
void CurlUtil::Post( const std::string &sPath, Json::Value &data, dxx_http_callback cb )
{
	...
	// Private handle
	dxx_curl_handle *pHandle = new dxx_curl_handle;
	pHandle->sPost = writer.write( data );
	pHandle->cb = cb;
	...
	curl_easy_setopt( pCurl, CURLOPT_POSTFIELDS, pHandle->sPost.c_str() );
	curl_easy_setopt( pCurl, CURLOPT_POSTFIELDSIZE, pHandle->sPost.length() );
	curl_easy_setopt( pCurl, CURLOPT_WRITEFUNCTION, read_cb );
	curl_easy_setopt( pCurl, CURLOPT_WRITEDATA, pHandle );
	curl_easy_setopt( pCurl, CURLOPT_PRIVATE, pHandle );
...
void CurlUtil::Tick()
{
	...
	// Now, iterate through some and find ones that are done
	...
	while( pInfo != NULL )
	{
		...
		// Handle dis, do magic
		if( reader.parse( std::string( pBuf ), val ) )
			pHandle->cb( val );
		else
